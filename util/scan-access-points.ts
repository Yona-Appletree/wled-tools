    ...
    function doScan() {
      exec('airport --xml -s', (error, stdout, stderr) => {
        if (stdout.trim().length === 0) {
          ...
          const rawApList = plist.parse(
            stdout,
          ) as unknown as Array<RawAccessPointInfo>;
          ...
          resolve(
            rawApList
              .map((it) => ({
                ...
                raw: it,
                isLikelyEsp32:
                  it.RSN_IE?.IE_KEY_RSN_MCIPHER === 2 &&
                  it['80211D_IE']?.IE_KEY_80211D_COUNTRY_CODE === 'CN',
              }))
...
export async function connectToAp(ssid: string, password: string) {
